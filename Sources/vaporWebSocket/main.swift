...
//     })
//
//class R : HTTPServerResponder {
//    let w = MultiThreadedEventLoopGroup.init(numberOfThreads: 1)
//
//    func respond(to request: HTTPRequest, on worker: Worker) -> Future<HTTPResponse> {
//        print("respond")
//        let promise : EventLoopPromise<HTTPResponse> = self.w.next().newPromise()
//        promise.succeed(result: HTTPResponse.init())
//        return promise.futureResult
//    }
//}
//
//let worker = MultiThreadedEventLoopGroup.init(numberOfThreads: 2)
//
...
import NIOHTTP1
import NIOWebSocket
// класс с обработчиками типа onText от Vapor для апгрейдера
import WebSocket
...
// старый апгрейдер от чувака из эпл - без няшных методов типа onText и тд от Vapor
